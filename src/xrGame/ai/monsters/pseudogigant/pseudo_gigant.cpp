...
void CPseudoGigant::Load(LPCSTR section)
{
    ...
    GEnv.Sound->create(m_sound_threaten_hit, pSettings->r_string(section, "sound_threaten_hit"), st_Effect, SOUND_TYPE_WORLD);
    GEnv.Sound->create(m_sound_start_threaten, pSettings->r_string(section, "sound_threaten_start"), st_Effect, SOUND_TYPE_MONSTER_ATTACKING);
...
void CPseudoGigant::event_on_step()
{
    ...
    if (pActor)
    {
        ...
        if (dist_to_actor < max_dist)
            Actor()->Cameras().AddCamEffector(new CPseudogigantStepEffector(step_effector.time, step_effector.amplitude, step_effector.period_number, (max_dist - dist_to_actor) / (1.2f * max_dist)));
    }
    //////////////////////////////////
}
